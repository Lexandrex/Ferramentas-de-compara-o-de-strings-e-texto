  ...
  search(text, pattern, stepByStep = false) {
    ...
    for (let i = 0; i <= n - m; i++) {
      ...
      for (let j = 0; j < m; j++) {
        comparisons++;
        if (stepByStep) {
          steps.push({ algorithm: 'Naive', i, j, textChar: text[i + j], patternChar: pattern[j], comparisons });
        }
    ...
    for (let i = 0; i <= n - m; i++) {
      if (p === t) {
        ...
        for (let j = 0; j < m; j++) {
          comparisons++;
          if (stepByStep) {
            steps.push({ algorithm: 'Rabin-Karp', i, j, textChar: text[i + j], patternChar: pattern[j], comparisons });
          }
          if (text[i + j] !== pattern[j]) {
            match = false;
            break;
...
export class KMPSearch extends SearchStrategy {
  ...
  search(text, pattern, stepByStep = false) {
    ...
    while (i < n) {
      comparisons++;
      if (stepByStep) {
        steps.push({ algorithm: 'KMP', i, j, textChar: text[i], patternChar: pattern[j], comparisons, lps: [...lps] });
      }
      if (text[i] === pattern[j]) {
        i++;
        j++;
  ...
  search(text, pattern, stepByStep = false) {
    ...
    while (s <= n - m) {
      ...
      while (j >= 0) {
        comparisons++;
        if (stepByStep) {
          steps.push({ algorithm: 'Boyer-Moore', s, j, textChar: text[s + j], patternChar: pattern[j], comparisons, badChar });
        }
